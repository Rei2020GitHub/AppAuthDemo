...
import com.sample.hmssample.appauthdemo.model.auth.FacebookAppAuth
...
class MainViewModel(
    ...
    private val facebookAuth: BaseAuth = FacebookAppAuth.getInstance()
    ...
    fun signInFacebookId(fragment: Fragment) {
        facebookAuth.signIn(
            fragment,
            object : SignInCallback {
                override fun signInSuccess(userInfo: UserInfo) {
                    ...
                    addLog("[" + facebookAuth.javaClass.simpleName + "] Sign in Facebook Id success")
                }

                override fun signInFail(exception: Exception?) {
                    addLog("[" + facebookAuth.javaClass.simpleName + "] Sign in Facebook Id fail")
                    exception?.message?.let {
                        addLog(it)
                    }
                }
    ...
    fun onActivityResult(context: Context, requestCode: Int, resultCode: Int, data: Intent?) {
        data?.let { data ->
            when (requestCode) {
                ...
                FacebookAppAuth.REQUEST_CODE -> {
                    facebookAuth.onActivityResult(context, requestCode, resultCode, data)
                }
            }
        }
    }
